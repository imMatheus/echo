...
const AUTH_TOKEN_SECRET = process.env.AUTH_TOKEN_SECRET || 'echo-development-auth-token-secret-change-before-deploying';
...
const sigA = await api('POST', '/auth/signup', {
  body: { email: aliceEmail, password: 'password-alice', name: 'Alice' },
});
check('signup alice 200', sigA.status === 200, JSON.stringify(sigA.json));
check(
  'signup requires verification and has no session',
  sigA.json?.verificationRequired === true && !cookieFrom(sigA.res),
);

const unverifiedLogin = await api('POST', '/auth/login', {
  body: { email: aliceEmail, password: 'password-alice' },
});
check(
  'unverified login blocked',
  unverifiedLogin.status === 403 && unverifiedLogin.json?.error?.code === 'email_not_verified',
);
const verifyA = await api('POST', '/auth/verify-email', {
  body: { token: await authToken(aliceEmail, 'verify_email') },
});
let cookieA = cookieFrom(verifyA.res);
check('alice verification creates session', verifyA.status === 200 && cookieA.length > 20);

const sigB = await api('POST', '/auth/signup', {
  body: { email: bobEmail, password: 'password-bob!', name: 'Bob' },
});
check('signup bob 200', sigB.status === 200);
const verifyB = await api('POST', '/auth/verify-email', {
  body: { token: await authToken(bobEmail, 'verify_email') },
});
...
const unknownReset = await api('POST', '/auth/forgot-password', {
  body: { email: `missing-${RUN}@example.com` },
});
...
const reset = await api('POST', '/auth/reset-password', {
  body: { token: resetToken, password: 'new-password-alice' },
});
check('password reset succeeds', reset.status === 200);
const resetReuse = await api('POST', '/auth/reset-password', {
  body: { token: resetToken, password: 'another-password' },
});
check(
  'password reset token is one-time',
  resetReuse.status === 400 && resetReuse.json?.error?.code === 'password_reset_invalid',
);
...
const reloginA = await api('POST', '/auth/login', {
  body: { email: aliceEmail, password: 'new-password-alice' },
});
...
const m1 = await api('POST', '/memories', {
  cookie: cookieA,
  body: {
    content: 'Alice prefers TypeScript strict mode and pnpm for all new projects',
    tags: ['preferences', 'tooling'],
  },
});
check('create memory 201', m1.status === 201, JSON.stringify(m1.json));
check(
  'memory defaults: explicit, personal scope, dashboard source',
  m1.json?.memory?.kind === 'explicit' &&
    m1.json?.memory?.scopeId === aliceScope &&
    m1.json?.memory?.sourceApp === 'dashboard',
);

const m2 = await api('POST', '/memories', {
  cookie: cookieA,
  body: {
...
check(
  'create high-sensitivity inferred memory',
  m2.status === 201 && m2.json.memory.sensitivity === 'high' && m2.json.memory.confidence === 0.8,
);

const m3 = await api('POST', '/memories', {
  cookie: cookieA,
  body: {
    content: 'Currently traveling in Portugal until August',
    expiresAt: new Date(Date.now() + 86400e3).toISOString(),
  },
});
check('create expiring memory', m3.status === 201 && !!m3.json.memory.expiresAt);

const pastExpiry = await api('POST', '/memories', {
  cookie: cookieA,
  body: { content: 'Already stale', expiresAt: new Date(Date.now() - 1000).toISOString() },
});
...
const search = await api('POST', '/memories/search', {
  cookie: cookieA,
  body: { query: 'typescript tooling' },
});
check('search returns fts mode (no provider)', search.json?.mode === 'fts', JSON.stringify(search.json));
check(
  'search finds the tooling memory first',
  search.json?.results?.[0]?.content?.includes('TypeScript'),
  JSON.stringify(search.json?.results?.map((r) => r.content)),
);

const patch = await api('PATCH', `/memories/${m1.json.memory.id}`, {
  cookie: cookieA,
  body: { tags: [' Preferences ', 'tooling', 'TOOLING', 'TypeScript'], confidence: 0.95 },
});
check(
  'patch normalizes and deduplicates tags',
  patch.status === 200 &&
    JSON.stringify(patch.json.memory.tags) === JSON.stringify(['preferences', 'tooling', 'typescript']) &&
    patch.json.memory.confidence === 0.95,
);

const shortLived = await api('POST', '/memories', {
  cookie: cookieA,
  body: {
    content: 'Short-lived exact-read check',
...
const eraseExpired = await api('DELETE', `/memories/${shortLived.json.memory.id}`, {
  cookie: cookieA,
});
check(
  'exact delete permanently erases an expired retained memory',
  eraseExpired.status === 200,
  `status=${eraseExpired.status}`,
);
...
const bobSearch = await api('POST', '/memories/search', {
  cookie: cookieB,
  body: { query: 'peanuts allergy' },
});
...
const addBob = await api('POST', `/orgs/${orgId}/members`, {
  cookie: cookieA,
  body: { email: bobEmail, role: 'member' },
});
check('add bob as member 201', addBob.status === 201);

const ghost = await api('POST', `/orgs/${orgId}/members`, {
  cookie: cookieA,
  body: { email: 'ghost@example.com' },
});
check('adding unknown email 404', ghost.status === 404);

const orgMem = await api('POST', '/memories', {
  cookie: cookieA,
  body: {
    content: 'BigQuery table analytics.events_v3 is the canonical event stream',
    scopeId: orgScope.id,
    tags: ['bigquery'],
  },
});
...
check(
  'bob sees exactly the org memory',
  bobList2.json?.total === 1 && bobList2.json?.memories?.[0]?.scopeType === 'organization',
);

const bobEdit = await api('PATCH', `/memories/${orgMem.json.memory.id}`, {
  cookie: cookieB,
  body: { content: 'vandalized' },
});
check('bob (member, not creator) cannot edit org memory (403)', bobEdit.status === 403, `status=${bobEdit.status}`);

const team = await api('POST', '/scopes', {
  cookie: cookieA,
  body: { orgId, type: 'team', name: 'Platform' },
});
check('create team scope 201', team.status === 201);
const teamMem = await api('POST', '/memories', {
  cookie: cookieA,
  body: { content: 'Platform team deploys via ArgoCD on Fridays', scopeId: team.json.scope.id },
});
...
check(
  'organization scope lists organization members',
  orgScopeMembers.status === 200 && orgScopeMembers.json?.members?.length === 2,
);

const hiddenTeamMembers = await api('GET', `/scopes/${team.json.scope.id}/members`, {
  cookie: cookieB,
});
check(
  'inaccessible nested scope does not leak members (404)',
  hiddenTeamMembers.status === 404,
  `status=${hiddenTeamMembers.status}`,
);
...
const addBobTeam = await api('POST', `/scopes/${team.json.scope.id}/members`, {
  cookie: cookieA,
  body: { email: bobEmail },
});
...
check(
  'bob now sees team scope',
  bobScopes2.json?.scopes?.some((s) => s.id === team.json.scope.id),
);

const demote = await api('PATCH', `/orgs/${orgId}/members/${meA.json.user.id}`, {
  cookie: cookieA,
  body: { role: 'member' },
});
...
const key = await api('POST', '/api-keys', {
  cookie: cookieA,
  body: { name: 'Claude Code', sourceApp: 'claude-code' },
});
...
const bearerMint = await api('POST', '/api-keys', {
  bearer: secret,
  body: { name: 'persistence attempt' },
});
...
check(
  'mcp initialize ok',
  init.json?.result?.serverInfo?.name === 'echo-context',
  JSON.stringify(init.json ?? init.text).slice(0, 300),
);
...
check(
  ...
  JSON.stringify(toolNames) ===
    JSON.stringify(['forget_context', 'list_context', 'list_scopes', 'recall_context', 'remember_context']),
  JSON.stringify(toolNames),
);

const remember = await mcp(
  {
    jsonrpc: '2.0',
    id: 3,
    method: 'tools/call',
    params: {
      ...
      arguments: {
        content: 'Alice reviews PRs before standup every morning',
        kind: 'inferred',
        confidence: 0.7,
        tags: ['habits'],
      },
...
check(
  'mcp remember_context stores memory',
  rememberPayload.stored === true && rememberPayload.memory?.source_app === 'claude-code',
  remember.text?.slice(0, 300),
);

const recall = await mcp(
  {
    jsonrpc: '2.0',
    id: 4,
    method: 'tools/call',
    params: { name: 'recall_context', arguments: { query: 'standup habits' } },
  },
  secret,
);
const recallPayload = JSON.parse(recall.json?.result?.content?.[0]?.text ?? '{}');
check(
  'mcp recall_context finds it',
  recallPayload.count >= 1 && recallPayload.results?.some((r) => r.content.includes('standup')),
  recall.text?.slice(0, 300),
);

const scopesTool = await mcp(
  { jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'list_scopes', arguments: {} } },
  secret,
);
...
const uppercaseScopeRecall = await mcp(
  {
    jsonrpc: '2.0',
    id: 51,
    method: 'tools/call',
    params: {
      name: 'recall_context',
      arguments: { query: 'canonical event stream', scope: orgScope.id.toUpperCase() },
    },
  },
  secret,
);
...
check(
  'MCP accepts uppercase scope UUIDs',
  uppercaseScopePayload.results?.some((r) => r.id === orgMem.json.memory.id),
  uppercaseScopeRecall.text?.slice(0, 300),
);

const rememberOrg = await mcp(
  {
    jsonrpc: '2.0',
    id: 6,
    method: 'tools/call',
    params: {
      name: 'remember_context',
      arguments: { content: 'Deploys are frozen during December', scope: `Acme ${RUN}` },
    },
  },
  secret,
);
...
check(
  'mcp remember into org scope by name',
  rememberOrgPayload.memory?.scope?.includes('organization'),
  rememberOrg.text?.slice(0, 200),
);

const forget = await mcp(
  {
    jsonrpc: '2.0',
    id: 7,
    method: 'tools/call',
    params: { name: 'forget_context', arguments: { memory_id: rememberPayload.memory.id } },
  },
  secret,
);
...
check(
  'personal audit has creates/recalls/deletes',
  actions.includes('memory.create') && actions.includes('memory.recall') && actions.includes('memory.delete'),
  JSON.stringify(actions.slice(0, 10)),
);
...
check(
  'org audit shows org events',
  orgActions.includes('memory.create') && orgActions.includes('org.member_add'),
  JSON.stringify(orgActions.slice(0, 10)),
);
...
check(
  'unscoped API list is audited per org without filters',
  !!orgList && orgList.details?.orgFanout === true && orgList.details?.filters === undefined,
);
...
const promoteBob = await api('PATCH', `/orgs/${orgId}/members/${verifyB.json.user.id}`, {
  cookie: cookieA,
  body: { role: 'owner' },
});
check('promote bob to owner', promoteBob.status === 200);
const concurrentDemotions = await Promise.all([
  api('PATCH', `/orgs/${orgId}/members/${meA.json.user.id}`, {
    cookie: cookieA,
    body: { role: 'member' },
  }),
  api('PATCH', `/orgs/${orgId}/members/${verifyB.json.user.id}`, {
    cookie: cookieB,
    body: { role: 'member' },
  }),
]);
...
check(
  'concurrent demotions preserve one owner',
  JSON.stringify(demotionStatuses) === JSON.stringify([200, 400]),
  JSON.stringify(demotionStatuses),
);
const membersAfterRace = await api('GET', `/orgs/${orgId}/members`, { cookie: cookieA });
check(
  'organization still has exactly one owner',
  membersAfterRace.json?.members?.filter((member) => member.role === 'owner').length === 1,
);
