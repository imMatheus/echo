...
Two interchangeable mechanisms — every endpoint below accepts either unless noted:
...
- `GET /memories` → `ListMemoriesResponse` `{ memories, total }`
  - Query params (all optional): `scopeId`, `q` (substring filter), `kind`, `sensitivity`, `sourceApp`, `tag`, `limit` (default 50, max 200), `offset`.
...
- `DELETE /memories/:id` → `{ ok: true }` — soft delete; same permission as PATCH.
...
- `GET /orgs/:id/audit?limit&offset&action` → `AuditListResponse` — admin/owner only. Org-scoped events only; personal memories never appear here.
...
- Clients that only support stdio can use the `echo-context-mcp` bridge package (see `packages/mcp-bridge`).
